...
const ChatAnalistaClienteEmbedded = ({ ticketId, onBack }) => {
    const { store, dispatch, joinTicketRoom, leaveTicketRoom, joinChatAnalistaCliente, leaveChatAnalistaCliente } = useGlobalReducer();
        ...
        if (ticketId && store.websocket.socket && store.websocket.connected) {
            ...
            const handleNuevoMensaje = (data) => {
                if (data.ticket_id === parseInt(ticketId)) {
                    console.log('💬 CHAT ANALISTA-CLIENTE - NUEVO MENSAJE RECIBIDO:', data);
                    ...
                    setTimeout(() => {
                        console.log('🔄 CHAT ANALISTA-CLIENTE - Segunda actualización (300ms)');
                        cargarMensajes(false);
                    }, 300);
                    setTimeout(() => {
                        console.log('🔄 CHAT ANALISTA-CLIENTE - Tercera actualización (1000ms)');
                        cargarMensajes(false);
                    }, 1000);
                }
            };
    ...
    useEffect(() => {
        ...
        const handleRealtimeChat = (event) => {
            ...
            if (data.ticket_id === parseInt(ticketId)) {
                console.log('💬 CHAT ANALISTA-CLIENTE EMBEBIDO - Evento realtime recibido desde página principal');
                setSincronizando(true);
                cargarMensajes(false).finally(() => setSincronizando(false));
    ...
    return (
        <div className="container-fluid py-4">
                ...
                <div className="col-12">
                    <div className="d-flex align-items-center justify-content-between mb-3">
                        <div className="d-flex align-items-center gap-3">
                            ...
                            <div>
                                <h1 className="mb-0 fw-bold">Chat Analista - Cliente - Ticket #{ticketId}</h1>
                                <p className="text-muted mb-0">Conversación directa con el analista</p>
                            </div>
                        </div>
                        <div className="d-flex align-items-center gap-2">
                            <span className="badge bg-primary me-2">{mensajes.length} mensajes</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="row justify-content-center">
                <div className="col-md-8 col-lg-6 col-xl-4">
                    <div className="card">
                        <div className="card-header p-3" style={{ borderTop: '4px solid #0d6efd' }}>
                            ...
                            <div className="d-flex align-items-center">
                                ...
                                <div className="d-flex align-items-center">
                                    <div className="rounded-circle d-flex align-items-center justify-content-center text-white bg-success me-2"
                                        style={{ width: '25px', height: '25px' }}>
                                        <i className="fas fa-user" style={{ fontSize: '12px' }}></i>
                                    </div>
                                    <small className="text-muted">{nombresParticipantes.cliente}</small>
                                </div>
                            </div>
                        </div>
...
export default ChatAnalistaClienteEmbedded;
